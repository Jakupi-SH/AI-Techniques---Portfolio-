...
public class DFS_Iterative {
    ...
    static void runTest(int size, double obstacleChance) {
        ...
        boolean found = dfsIterative(0, 0, rows - 1, cols - 1);

        long end = System.nanoTime();

        System.out.println("\nITERATIVE DFS TEST " + size + "x" + size);
        System.out.println("Found: " + found);
        System.out.println("Nodes visited: " + nodesVisited);
        System.out.println("Time ms: " + (end - start) / 1_000_000);
    }

    static boolean dfsIterative(int sr, int sc, int gr, int gc) {
        ...
        while (!stack.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                stack.push(new Node(cur.r + dRow[i], cur.c + dCol[i]));
            }
        }
